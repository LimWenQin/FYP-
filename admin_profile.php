...
// --- Handle Change Password Logic (Independent Modal - Final Step) ---
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['change_password_action'])) {
    $currentPass = $_POST['current_password'];
    $newPass = $_POST['new_password'];
    $confirmPass = $_POST['confirm_new_password'];

    // PHP Validation Regex (Matching Add Donor / Dashboard)
    $pattern = '/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()\-_=+{};:,<.>])[A-Za-z\d!@#$%^&*()\-_=+{};:,<.>]{8,15}$/';

    if ($newPass !== $confirmPass) {
        $error = "New passwords do not match.";
    } elseif (!preg_match($pattern, $newPass)) {
        $error = "Password does not meet requirements (8-15 chars, uppercase, lowercase, number, special char).";
    } else {
        // 1. 获取数据库中当前的密码
        $stmt = $conn->prepare("SELECT Admin_Password FROM admin WHERE Admin_ID = ?");
        $stmt->bind_param("i", $adminId);
        ...
        if ($result->num_rows > 0) {
            ...
            // 2. 验证旧密码
            $isPasswordCorrect = password_verify($currentPass, $dbPass) || ($currentPass === $dbPass);

            if ($isPasswordCorrect) {
                 // 3. 旧密码正确，更新新密码 (使用 Hash 加密)
...
<head>
    ...
    <style>
        /* --- HEADER & SIDEBAR STYLES (MATCHING DASHBOARD) --- */
        .top-nav { height: 80px; display: flex; align-items: center; justify-content: space-between; padding: 0 30px; background: white; box-shadow: 0 2px 5px rgba(0,0,0,0.05); position: sticky; top: 0; z-index: 100; }
        .logo a { display: flex; align-items: center; gap: 15px; text-decoration: none; }
        .logo img { height: 60px; width: auto; object-fit: contain; }
        .logo h1 { font-size: 24px; font-weight: 700; color: #F28585; margin: 0; line-height: 1; }
        ...
        /* Helper Text & Form Guides */
        .form-guide { font-size: 11px; color: #6c757d; margin-top: 4px; display: block; font-style: italic; }
        ...
        /* --- UPDATED MODAL STYLES (PRETTIER & CLEANER) --- */
        ...
        /* Fancy Input Group for Password */
        ...
        .confirm-check { position: absolute; right: 50px; top: 50%; transform: translateY(-50%); color: #28a745; display: none; z-index: 5; font-size: 16px; }
        .required { color: #dc3545; font-weight: bold; margin-left: 3px; }
        
        /* New Styled Buttons */
        .modal-btn-primary {
            width: 100%;
            padding: 12px;
            background: #F28585; /* 主题色 */
            color: white;
            border: none;
            border-radius: 8px;
            font-size: 15px;
...
<body>
            ...
            <div class="profile-container">
                <form method="POST" enctype="multipart/form-data" onsubmit="return validateProfileForm()">
                    ...
                    <div class="form-group">
                        ...
                        <?php if($editMode): ?>
                            <span class="form-guide">Only English letters and spaces are allowed. e.g. John Doe</span>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <?php if($editMode): ?>
                                <span class="form-guide">Example: user@example.com. Must include '@' and end with .com, .net, etc.</span>
                        ...
                        <div class="form-group">
                            ...
                            <?php if ($editMode): ?>
                                ...
                                <span class="form-guide">(e.g., +6012-3456789). Do not enter +601 again.</span>
                            <?php else: ?>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <?php if($editMode): ?><span class="form-guide">e.g. 900101-01-1234</span><?php endif; ?>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="address1" class="form-input" 
                               ...
                               <?php echo $editMode ? 'required' : 'readonly'; ?>>
                        <?php if($editMode): ?><span class="form-guide">House number, street name.</span><?php endif; ?>
                    </div>
                    <div class="form-group">
    ...
    <script>
        ...
        function validateForm() {
            return validatePasswordRequirements() && validatePasswordMatch();
        }

        function resetRequirements() {
            const items = document.querySelectorAll('.requirement-item');
            items.forEach(item => {
                item.className = 'requirement-item invalid';
                item.querySelector('i').className = 'fas fa-times';
            });
            document.getElementById("password-match-icon").style.display = "none";
            document.getElementById("passError").style.display = "none";
        }

        // --- ENTER KEY LOGIC & URL CHECK ---
        document.addEventListener("DOMContentLoaded", function() {
            
            // 1. Auto Open from URL
            const urlParams = new URLSearchParams(window.location.search);
            if (urlParams.get('action') === 'change_password') {
                openChangePasswordModal();
            }

            // 2. Enter Key Logic for Step 1
            ...
            if(currentPassInput) {
                currentPassInput.addEventListener('keypress', function(e) {
                    if (e.key === 'Enter') {
                        ...
                        verifyCurrentPass(); // Click "Next"
                    }
                });
            }

            // 3. Enter Key Logic for Step 2 (Optional, usually default submit works, but let's be safe)
            const newPassInput = document.getElementById('new_password');
            const confirmPassInput = document.getElementById('confirm_new_password');
            
            // Allow Enter to submit form on last step
            confirmPassInput.addEventListener('keypress', function(e) {
                if(e.key === 'Enter') {
                    // Default behavior of form submit is fine here as it triggers validation
                }
            });
        });
