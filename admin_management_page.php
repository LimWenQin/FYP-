    ...
    // 验证年龄
        ...
        // 检查邮箱是否已存在
            ...
            if ($emailResult && $emailResult->num_rows > 0) {
                ...
                // 插入新管理员
                $sql = "INSERT INTO admin (Admin_FName, Admin_LName, Admin_ContactNumber, Admin_ICNumber, Admin_Email, Admin_Password, Admin_Address, Admin_DOB, Admin_Position) 
                        VALUES ('$firstName', '$lastName', '$contact', '$icNumber', '$email', '$hashedPassword', '$address', '$dob', '$position')";
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql = "SELECT * FROM admin 
            WHERE Admin_FName LIKE '%$searchTerm%' 
            OR Admin_LName LIKE '%$searchTerm%' 
            OR Admin_Email LIKE '%$searchTerm%' 
            OR Admin_ID LIKE '%$searchTerm%'
            OR Admin_Position LIKE '%$searchTerm%'
    ...
    $sql = "SELECT * FROM admin ORDER BY Admin_FName LIMIT $start_from, $results_per_page";
...
function getActiveAdmins($conn) {
    // 修复：移除不存在的 Admin_LastLogin 字段的查询条件
    $sql = "SELECT COUNT(*) as total FROM admin";
    $result = $conn->query($sql);
    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
...
$activeAdmins = getActiveAdmins($conn);
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="dashboard-content">
            <div class="welcome-section">
                ...
                <p>Manage all administrators, view details, and assign roles.</p>
            </div>
            ...
            <div class="stats-cards">
                ...
                <div class="stat-card">
                    <div class="stat-info">
                        <h3>ACTIVE ADMINS</h3>
                        <h2><?php echo $activeAdmins; ?></h2>
                        <p><i class="fas fa-arrow-up"></i> +3.2% from last month</p>
                    </div>
                    <div class="stat-icon">
            ...
            <div class="admin-management">
                ...
                <form method="GET" action="admin_management_page.php" class="admin-search">
                    <input type="text" name="search" class="search-input" placeholder="Search admins by name, ID, email or position..." value="<?php echo htmlspecialchars($searchTerm); ?>">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-search"></i> Search
                    </button>
                    <?php if (!empty($searchTerm)): ?>
                        <a href="admin_management_page.php" class="btn btn-danger">
                            ...
                            <tr>
                                <td>
                                    <div class="admin-info">
                                        <div class="admin-avatar"><?php echo substr($admin['Admin_FName'], 0, 1); ?></div>
                                        <div class="admin-details">
                                            <h4><?php echo htmlspecialchars($admin['Admin_FName'] . ' ' . $admin['Admin_LName']); ?></h4>
                                            <p>ID: <?php echo htmlspecialchars($admin['Admin_ID']); ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="admin-details">
                                        ...
                                        <p><?php echo htmlspecialchars($admin['Admin_ContactNumber']); ?></p>
                                    </div>
                                </td>
                                <td><?php echo htmlspecialchars($admin['Admin_Position'] ?? 'System Administrator'); ?></td>
                                <td>
                                    <?php 
                                    // Determine status based on last login (simplified logic)
                                    $status = "Active";
                                    ?>
                                ...
                                <td>
                                    <div class="action-cell">
                                        ...
                                        <button class="action-btn delete-btn" onclick="confirmDelete(<?php echo $admin['Admin_ID']; ?>, '<?php echo htmlspecialchars($admin['Admin_FName'] . ' ' . $admin['Admin_LName']); ?>')">
    ...
    <div class="modal" id="addAdminModal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addAdminForm" action="admin_management_page.php" method="POST" onsubmit="return validateForm()">
                    <input type="hidden" name="add_admin" value="1">
                    <div class="form-row">
                        <div class="form-group">
                            <label class="form-label" for="first_name">First Name</label>
                            <input type="text" id="first_name" name="first_name" class="form-input" required>
                            <span class="form-guide">Enter admin's first name (e.g., John)</span>
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="last_name">Last Name</label>
                            <input type="text" id="last_name" name="last_name" class="form-input" required>
                            <span class="form-guide">Enter admin's last name (e.g., Smith)</span>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <span class="form-guide">Admin must be at least 18 years old</span>
                            <div id="ageError" class="error-message">Admin must be at least 18 years old</div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <span class="form-guide">Enter admin position (e.g., System Administrator, Branch Manager)</span>
                    </div>
    ...
    <script>
        ...
        function confirmDelete(adminId, adminName) {
            if (confirm('Are you sure you want to delete admin "' + adminName + '"? This action cannot be undone.')) {
                window.location.href = 'admin_management_page.php?delete_id=' + adminId;
            }
        }
