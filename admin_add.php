...
// Check permissions (Optional: Only Super Admin can add other admins)
$currentAdminId = $_SESSION['admin_id'];
$currentUserRole = 'Admin'; 
$roleRes = $conn->query("SELECT Admin_Role FROM admin WHERE Admin_ID = $currentAdminId");
if ($roleRes && $row = $roleRes->fetch_assoc()) $currentUserRole = $row['Admin_Role'];

if ($currentUserRole !== 'Super Admin') {
    header("Location: admin_management_page.php?error=" . urlencode("Permission Denied. Only Super Admin can add new admins."));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_admin'])) {
    ...
    if (empty($fullName) || empty($email) || empty($contactRaw) || empty($icNumber) || empty($dob)) {
        ...
        if ($emailExists) {
            ...
            $sql = "INSERT INTO admin (Admin_Name, Admin_ContactNumber, Admin_ICNUMBER, Admin_Email, Admin_Password, Admin_IsFirstLogin, Admin_DOB, Admin_Address1, Admin_Address2, Admin_Address3, Admin_City, Admin_State, Admin_PostalCode, Admin_Country, Admin_Comment, Admin_Role, Admin_Status, Admin_ProfilePicture, Admin_CreatedAt) VALUES ('$fullName', '$contact', '$icNumber', '$email', '$hashedPassword', 1, '$dob', '$address1', '$address2', '$address3', '$city', '$state', '$postalCode', '$country', '$comment', '$role', '$status', $dbProfilePic, NOW())";
            ...
            if ($conn->query($sql)) {
                ...
                try {
                    ...
                    $mail->isHTML(true); $mail->Subject = 'Welcome to Love Bridge Admin Panel';
                    $mail->Body = "<h2>Welcome $fullName!</h2><p>You have been registered as an <b>$role</b>.</p><p>Your temp password: <b>$rawPassword</b></p>";
                    $mail->send();
                } catch (Exception $e) {
                    // Email failed but admin created
                }
                $saveSuccess = true;
...
<head>
    ...
    <title>Add Admin - Donation Management System</title>
...
<body>
    ...
    <div id="successModal" class="modal-overlay" style="display: <?php echo $saveSuccess ? 'flex' : 'none'; ?>;">
        <div class="success-modal">
            ...
            <p style="color: #666; line-height: 1.5;">New Admin has been successfully added to the system.</p>
            <div class="modal-btn-group">
                <a href="admin_management_page.php" class="btn-clear" style="border: 1px solid #ddd;">Return to List</a>
        ...
        <div class="dashboard-content" style="padding-top: 10px;">
            <div class="page-header-compact">
                ...
                <div class="header-title">
                    <h1>Add New Admin</h1>
                    <p>Create a new administrator account with specific roles.</p>
                </div>
            </div>

            <div class="form-container">
                <form id="addAdminForm" action="admin_add.php" method="POST" enctype="multipart/form-data" onsubmit="return validateForm('add')" novalidate>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="full_name" name="full_name" class="form-input" required oninput="validateName(this)" placeholder="e.g. Admin Name">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="email" name="email" class="form-input" required placeholder="e.g. admin@example.com">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="comment" class="form-textarea" rows="3" placeholder="e.g. Admin responsibilities or department"></textarea>
                        <span class="form-guide">Optional notes about this administrator.</span>
                    </div>

                    <div class="button-group">
                        <button type="button" class="btn-clear" onclick="clearFormCustom()"><i class="fas fa-eraser"></i> Clear</button>
                        <button type="submit" class="btn-save"><i class="fas fa-save"></i> Save Admin</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function validateForm(type) {
            ...
            let hasError = false;
            let firstErrorMsg = "";
            let nameId='full_name', emailId='email', dobId='dob', contactId='add_contact', icId='ic_number';
            ...
            else if (!isAgeValid(dobVal)) { showFieldError(dobId, "Admin must be at least 18 years old."); hasError = true; if(!firstErrorMsg) firstErrorMsg="Admin must be > 18."; }

            if (hasError) { showSystemAlert(firstErrorMsg || "Please correct the highlighted errors.", 'error'); return false; }
            return true; 
        }
        ...
        function removeImage(inputId, containerId) {
            ...
            const container = document.getElementById(containerId);
            container.innerHTML = `<div class="default-avatar-icon"><i class="fas fa-user-shield"></i></div><button type="button" id="removeImgBtn" class="remove-img-btn" style="display: none;" onclick="removeImage('${inputId}', '${containerId}')"><i class="fas fa-times"></i></button>`;
