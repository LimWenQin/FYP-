...
if (isset($_GET['restore_id'])) {
    ...
    header("Location: admin_blocked_admins.php?success=" . urlencode("Admin restored successfully!")); 
    exit();
}
if (isset($_GET['delete_id'])) {
    $deleteId = intval($_GET['delete_id']);
    $conn->query("DELETE FROM admin WHERE Admin_ID = $deleteId");
    header("Location: admin_blocked_admins.php?success=" . urlencode("Admin permanently deleted!")); 
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bulk_action'])) {
    ...
    $ids_str = $_POST['admin_ids']; // Comma separated string
    ...
    if (!empty($ids_str)) {
        ...
        if ($action === 'bulk_restore') {
            $conn->query("UPDATE admin SET Is_Deleted = 0 WHERE Admin_ID IN ($ids_list)");
            $msg = count($safe_ids) . " admins restored successfully.";
        } elseif ($action === 'bulk_delete') {
            $conn->query("DELETE FROM admin WHERE Admin_ID IN ($ids_list)");
            $msg = count($safe_ids) . " admins deleted permanently.";
        }
        
        header("Location: admin_blocked_admins.php?success=" . urlencode($msg));
        exit();
    }
}
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $whereConditions[] = "(Admin_Name LIKE '%$searchTerm%' 
                            OR Admin_Email LIKE '%$searchTerm%' 
                            OR Admin_ID LIKE '%$searchTerm%'
                            OR Admin_ContactNumber LIKE '%$searchTerm%'
...
if (isset($_GET['filter_type']) && !empty($_GET['filter_type'])) {
    ...
    elseif ($filterType == 'phone' && isset($_GET['filter_val_phone']) && !empty($_GET['filter_val_phone'])) {
        ...
        $whereConditions[] = "Admin_ContactNumber LIKE '%$filterValue%'";
    }
    elseif ($filterType == 'role' && isset($_GET['filter_val_role']) && !empty($_GET['filter_val_role'])) {
        ...
        $whereConditions[] = "Admin_Role = '$filterValue'";
    }
}
...
$sql = "SELECT * FROM admin $whereClause $orderClause";
...
<body>
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="dashboard-content">
            <div class="blocked-table-container">
                ...
                <form class="search-filter-container" method="GET" action="admin_blocked_admins.php" style="display:flex; flex-wrap:wrap; align-items: flex-start;">
                    ...
                    <div class="filter-group" style="flex-wrap: wrap;">
                        ...
                        <div id="filter_role_container" class="secondary-filter">
                            <select name="filter_val_role" class="filter-select">
                                ...
                                <option value="Super Admin" <?php echo ($filterValue == 'Super Admin') ? 'selected' : ''; ?>>Super Admin</option>
                                <option value="Admin" <?php echo ($filterValue == 'Admin') ? 'selected' : ''; ?>>Admin</option>
                            </select>
                        </div>
                ...
                <form id="bulkForm" method="POST">
                    ...
                    <input type="hidden" name="admin_ids" id="bulkIdsInput">
                </form>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th onclick="openSortModal('nameSortModal')">
                                Administrator <?php if($sort=='name') echo ($order=='asc' ? '<i class="fas fa-sort-up"></i>' : '<i class="fas fa-sort-down"></i>'); else echo '<i class="fas fa-sort"></i>'; ?>
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr id="row-<?php echo $row['Admin_ID']; ?>">
                            <td class="checkbox-cell">
                                <input type="checkbox" class="row-checkbox" value="<?php echo $row['Admin_ID']; ?>" onclick="updateSelection()">
                            </td>
                            <td style="font-weight: bold; color: #666;">#<?php echo $row['Admin_ID']; ?></td>
                            <td>
                                <div class="user-info">
                                    <?php 
                                        $lightboxSrc = !empty($row['Admin_ProfilePicture']) ? htmlspecialchars($row['Admin_ProfilePicture']) : '';
                                    ?>
                            ...
                            <td class="action-cell">
                                <div class="action-buttons-wrapper">
                                    <button type="button" class="btn-icon btn-restore" title="Restore Admin" onclick="confirmSingleAction(<?php echo $row['Admin_ID']; ?>, 'restore')">
                                        <i class="fas fa-undo"></i>
                                    </button>
                                    <button type="button" class="btn-icon btn-delete" title="Delete Permanently" onclick="confirmSingleAction(<?php echo $row['Admin_ID']; ?>, 'delete')">
                        ...
                        <?php endwhile; else: ?>
                        <tr><td colspan="7" class="empty-state">No blocked admins found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
    ...
    <div id="roleSortModal" class="sort-modal" onclick="closeModal(event, 'roleSortModal')">
        <div class="sort-modal-content">
            ...
            <div class="prefix-grid" style="grid-template-columns: 1fr;"> <?php 
                $roles = ['Super Admin', 'Admin'];
                foreach($roles as $r) {
                    $url = buildUrl(['filter_type' => 'role', 'filter_val_role' => $r]);
                    $activeStyle = ($filterType == 'role' && $filterValue == $r) ? 'background-color:#f28585;color:white;border-color:#f28585;' : '';
                    echo "<a href='$url' class='prefix-btn' style='$activeStyle'>$r</a>";
                }
                ?>
            </div>
        </div>
    ...
    <script>
        ...
        function confirmSingleAction(id, type) {
            const title = (type === 'restore') ? 'Restore Admin?' : 'Delete Forever?';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `admin_blocked_admins.php?${urlParam}=${id}`;
                }
            });
        }
        ...
        function submitBatch(type) {
            ...
            const title = (type === 'restore') ? 'Restore Admins?' : 'Delete Forever?';
            const color = (type === 'restore') ? '#10b981' : '#dc3545';
            const text = (type === 'restore') 
                ? `Restore ${ids.length} admins?` 
                : `Permanently delete ${ids.length} admins? This cannot be undone.`;
